...
export function EditStudentForm({ studentToEdit, onSuccess, setDialogOpen }: EditStudentFormProps) {
  ...
  const form = useForm<CreateStudentFormInputValues>({
    ...
    defaultValues: {
      class_id: studentToEdit?.class_id?.toString() || undefined,
      roll_number: studentToEdit?.roll_number || '',
      full_name: studentToEdit?.full_name || '',
      dob: studentToEdit?.dob ? new Date(studentToEdit.dob) : undefined,
      gender: studentToEdit?.gender || undefined,
      email: studentToEdit?.email || '',
      phone: studentToEdit?.phone || '',
      address: studentToEdit?.address || '',
      admission_date: studentToEdit?.admission_date ? new Date(studentToEdit.admission_date) : new Date(),
    },
  });
  ...
  async function onSubmit(values: CreateStudentFormInputValues) {
    ...
    try {
      ...
      const result = await updateStudent(studentToEdit.student_id, validatedValues);

      if (result.success) {
        toast({
          title: 'Student Updated',
          description: result.message || `Student "${validatedValues.full_name}" has been successfully updated.`,
        });
        onSuccess?.();
        setDialogOpen(false);
      } else {
        toast({
          title: 'Update Failed',
          description: result.error || 'Could not update the student.',
          variant: 'destructive',
        });
      }
    } catch (error: any) {
       ...
       <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-4">
            ...
            render={({ field }) => (
                ...
                <FormControl>
                    <Input id="email" type="email" placeholder="e.g., lohith@gmail.com" disabled={isLoading} {...field} value={field.value ?? ''}/>
                </FormControl>
                <FormMessage />
                </FormItem>
            )}
            ...
            render={({ field }) => (
                ...
                <FormControl>
                    <Input id="phone" type="tel" placeholder="e.g., 9876543210" disabled={isLoading} {...field} value={field.value ?? ''}/>
                </FormControl>
                <FormMessage />
                </FormItem>
            )}
            ...
            render={({ field }) => (
                ...
                <FormControl>
                    <Textarea id="address" placeholder="Enter student's address" disabled={isLoading} {...field} value={field.value ?? ''}/>
                </FormControl>
                <FormMessage />
                </FormItem>
            )}
